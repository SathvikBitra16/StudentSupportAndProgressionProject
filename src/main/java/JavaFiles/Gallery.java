...
public class Gallery extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			PreparedStatement st = con.prepareStatement("INSERT INTO photo (image, about) VALUES (?, ?)");
			st.setString(1, inputFileName);
			st.setString(2, request.getParameter("about"));
			st.executeUpdate();

			response.sendRedirect("gallery.jsp"); // Redirect to gallery after successful upload
